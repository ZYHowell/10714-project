...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        if not node.requires_grad:
            kwargs = {"dtype": node.cached_data.dtype}
            if array_api is not numpy:
                kwargs["device"] = node.cached_data.device
            node.grad = Tensor.make_const(
                array_api.full(node.shape, 0, **kwargs))
            continue
        out_grads = node_to_output_grads_list[node]
        output_grad = out_grads[0]
        for g in out_grads[1:]:
            if isinstance(g, TensorTuple):
                assert isinstance(output_grad, TensorTuple)
                output_grad = needle.ops.make_tuple(*[
                    needle.ops.add(output_grad[i], g[i]) for i in range(len(g))
                ])
            else:
                output_grad = needle.ops.add(output_grad, g)
        node.grad = output_grad
        if node.is_leaf():
            continue
        grads = node.op.gradient_as_tuple(output_grad, node)
        for in_node, grad in zip(node.inputs, grads):
            node_to_output_grads_list.setdefault(in_node, []).append(grad)
    ### END YOUR SOLUTION
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    """
    ### BEGIN YOUR SOLUTION
    # TODO: this api results in a lot of redundant computations if the user
    # manually sets 'requires_grad' instead of using 'detach'.
    visited = set()
    topo_order = []
    is_used = set()
    for node in node_list:
        is_used.update(node.inputs)
    for node in node_list:
        if node not in is_used:
