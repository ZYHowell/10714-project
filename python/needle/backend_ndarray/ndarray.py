...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if np.prod(self.shape) != np.prod(new_shape):
            raise ValueError(
                f"Tensor with shape {self.shape} cannot be reshape to {new_shape}"
            )
        if not self.is_compact():
            raise ValueError("Tensor is not compact.")
    ...
    def broadcast_to(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        shape = self.shape
        assert all(new_shape[i] == shape[i] or shape[i] == 1
                   for i in range(self.ndim))
        diff, = np.nonzero(tuple(s != d for s, d in zip(new_shape, shape)))
    ...
    def __getitem__(self, idxs):
        ...
        indices = idxs
        new_shape = tuple([
            int(np.ceil((idx.stop - idx.start) / idx.step)) for idx in indices
        ])
        new_strides = tuple([
            idx.step * stride for idx, stride in zip(indices, self.strides)
        ])
        new_offset = sum(idx.start * stride
    ...
    def flip(self, axes):
        ...
        if isinstance(axes, int):
            axes = [axes]
        axes_set = set(axes)
        new_strides = tuple(-s if i in axes_set else s
                            for i, s in enumerate(self.strides))
        new_offset = sum(
            (self.shape[i] - 1) * self.strides[i] for i in axes) + self._offset
        return NDArray.make(self.shape, new_strides, self.device, self._handle,
                            new_offset).compact()
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        shape = tuple(le + l + r for le, (l, r) in zip(self.shape, axes))
        ret = full(shape, 0, dtype=self.dtype, device=self.device)

        indices = tuple(
            slice(l, l + le) for le, (l, _) in zip(self.shape, axes))
        ret[indices] = self
        return ret
        ### END YOUR SOLUTION
