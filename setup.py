...
def build():
    ...
    build_command += [f"CUDACXX={cudacxx_path} make"]
    build_command = " ".join(build_command)
    print(build_command)
    if subprocess.call(build_command, shell=True) != 0:
        print("Failed to build the ndarray backend")
        sys.exit()
...
if __name__ == "__main__":
    ...
    setup(
        name="needle",
        version="0.0.0",
        author="CMU 10-714 students",
        author_email="yhzhuang@cmu.edu",
        description="Needle is a project for CMU 10-714",
        ...
        keywords=("needle machine-learning deep-learning python"),
        packages=find_packages(exclude=["playground", "tests"]),
        python_requires='>=3.7',
