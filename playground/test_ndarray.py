...
_DEVICES = [
    nd.cpu(),
    pytest.param(nd.cuda(),
                 marks=pytest.mark.skipif(not nd.cuda().enabled(),
                                          reason="No GPU"))
]
...
@pytest.mark.parametrize(
    ...
    ids=[
        "transpose", "broadcast_to", "reshape1", "reshape2", "reshape3",
        "getitem1", "getitem2", "transposegetitem"
    ])
...
def test_reduce_sum(params, device):
    ...
    A = nd.array(_A, device=device)
    np.testing.assert_allclose(_A.sum(axis=axis),
                               A.sum(axis=axis).numpy(),
                               atol=1e-5,
                               rtol=1e-5)
...
def test_reduce_max(params, device):
    ...
    _A = np.random.randn(*dims)
    A = nd.array(_A, device=device)
    np.testing.assert_allclose(_A.max(axis=axis),
                               A.max(axis=axis).numpy(),
                               atol=1e-5,
                               rtol=1e-5)
...
def test_ewise_fn(fn, shape, device):
    ...
    A = nd.array(_A, device=device)
    B = nd.array(_B, device=device)
    np.testing.assert_allclose(fn(_A, _B),
                               fn(A, B).numpy(),
                               atol=1e-5,
                               rtol=1e-5)
...
def test_ewise_max(shape, device):
    ...
    A = nd.array(_A, device=device)
    B = nd.array(_B, device=device)
    np.testing.assert_allclose(np.maximum(_A, _B),
                               A.maximum(B).numpy(),
                               atol=1e-5,
                               rtol=1e-5)
...
matmul_dims = [(16, 16, 16), (8, 8, 8), (1, 2, 3), (3, 4, 5), (5, 4, 3),
               (64, 64, 64), (72, 72, 72), (72, 73, 74), (74, 73, 72),
               (128, 128, 128)]
...
def test_scalar_power(device):
    A = np.random.randn(5, 5)
    B = nd.array(A, device=device)
    np.testing.assert_allclose(np.power(A, 5.), (B**5.).numpy(),
                               atol=1e-5,
                               rtol=1e-5)
    np.testing.assert_allclose(np.power(A, 0.5), (B**0.5).numpy(),
                               atol=1e-5,
                               rtol=1e-5)
...
def test_scalar_maximum(device):
    ...
    B = nd.array(A, device=device)
    C = (np.max(A) + 1.).item()
    np.testing.assert_allclose(np.maximum(A, C), (B.maximum(C)).numpy(),
                               atol=1e-5,
                               rtol=1e-5)
    C = (np.max(A) - 1.).item()
    np.testing.assert_allclose(np.maximum(A, C), (B.maximum(C)).numpy(),
...
def test_ewise_log(device):
    ...
    B = nd.array(A, device=device)
    np.testing.assert_allclose(np.log(A), (B.log()).numpy(),
                               atol=1e-5,
                               rtol=1e-5)
...
def test_ewise_exp(device):
    A = np.random.randn(5, 5)
    B = nd.array(A, device=device)
    np.testing.assert_allclose(np.exp(A), (B.exp()).numpy(),
                               atol=1e-5,
                               rtol=1e-5)
...
def test_ewise_tanh(device):
    A = np.random.randn(5, 5)
    B = nd.array(A, device=device)
    np.testing.assert_allclose(np.tanh(A), (B.tanh()).numpy(),
                               atol=1e-5,
                               rtol=1e-5)
