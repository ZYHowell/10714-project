...
class IterTool {
 public:
  IterTool(const std::vector<int32_t>& shape,
           const std::vector<int32_t>& strides, int64_t offset)
      : _shape(shape), _strides(strides), _offset(offset) {
    dims = shape.size();
    iter.reserve(dims);
  ...
  int64_t next() {
    int iter_idx = dims - 1;
    do {
      if (iter[iter_idx] < _shape[iter_idx] - 1) {
        _offset += _strides[iter_idx];
        ++iter[iter_idx];
        break;
      } else {
        if (iter[iter_idx] > 0) {
          _offset -= _strides[iter_idx] * iter[iter_idx];
          iter[iter_idx] = 0;
        }
        --iter_idx;
      }
 ...
 private:
  const std::vector<int32_t> _shape;
  const std::vector<int32_t> _strides;
  int64_t _offset;
  int dims;
...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
   ...
   */
  /// BEGIN YOUR SOLUTION
  IterTool iter(shape, strides, offset);
  int64_t src_offset = iter.value();

  for (size_t dst_offset = 0; dst_offset < out->size; ++dst_offset) {
    out->ptr[dst_offset] = a.ptr[src_offset];
    src_offset = iter.next();
  }
  /// END YOUR SOLUTION
}
...
void EwiseSetitem(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
  ...
  /// BEGIN YOUR SOLUTION
  IterTool iter(shape, strides, offset);
  int64_t dst_offset = iter.value();

  for (size_t src_offset = 0; src_offset < a.size; ++src_offset) {
    out->ptr[dst_offset] = a.ptr[src_offset];
    dst_offset = iter.next();
  }
  /// END YOUR SOLUTION
}
...
void ScalarSetitem(const size_t size, scalar_t val, AlignedArray* out, std::vector<int32_t> shape,
  ...
  for (size_t i = 0; i < size && dst_offset >= 0; ++i) {
    out->ptr[dst_offset] = val;
    dst_offset = iter.next();
  }
  /// END YOUR SOLUTION
}
...
void Matmul(const AlignedArray& a, const AlignedArray& b, AlignedArray* out, uint32_t m, uint32_t n,
  ...
  for (size_t i = 0; i < m; ++i) {
    for (size_t j = 0; j < n; ++j) {
      const scalar_t num = a.ptr[i * n + j];
      for (size_t k = 0; k < p; ++k) {
        // result[i][k] += x[i][j] * y[j][k]
        out->ptr[i * p + k] += num * b.ptr[j * p + k];
      }
    }
  }
  /// END YOUR SOLUTION
