...
__global__ void EwiseSetKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
                               CudaVec strides, size_t offset) {
  /**
   * The CUDA kernel for the EwiseSet opeation. This should effectively map a single entry in the 
   * compact input a (at location gid), to the corresponding item in the non-compact array out.
   * 
   * Args:
   *   a: CUDA pointer to a array
   *   out: CUDA point to out array
...
__global__ void ScalarSetKernel(const scalar_t val, scalar_t* out, size_t size, CudaVec shape,
                                CudaVec strides, size_t offset) {
  /**
   * The CUDA kernel for the EwiseSet opeation. This should effectively map scalar
   * to the corresponding item in the non-compact array out.
   * 
   * Args:
   *   a: CUDA pointer to a array
   *   out: CUDA point to out array
...
#define BM BLOCK_SIZE
#define BN BLOCK_SIZE

__global__ void MatmulKernel(scalar_t* __restrict__ a, scalar_t* __restrict__ b,
                             scalar_t* __restrict__ c, uint32_t M, uint32_t N,
                             uint32_t K) {
...
// This one has a precision error for mugrade submission 3...
__global__ void MatmulKernel1(scalar_t* __restrict__ a, scalar_t* __restrict__ b,
                             scalar_t* __restrict__ c, uint32_t M, uint32_t N,
                             uint32_t K) {
  const size_t col = blockIdx.x * blockDim.x + threadIdx.x;
  const size_t row = blockIdx.y * blockDim.y + threadIdx.y;
  ...
  __shared__ scalar_t tile_a[BLOCK_SIZE][BLOCK_SIZE];
  __shared__ scalar_t tile_b[BLOCK_SIZE][BLOCK_SIZE];
  scalar_t tmp = 0;
  for (size_t i = 0; i < (K + BLOCK_SIZE - 1) / BLOCK_SIZE; ++i) {
    tile_a[ty][tx] = a[row * K + (i * BLOCK_SIZE) + tx];
    tile_b[ty][tx] = b[(i * BLOCK_SIZE + ty) * N + col];
    __syncthreads();
